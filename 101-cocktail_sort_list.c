...

	{
		...
		while (current->next != end && current->next != NULL)
		{
			if (current->n > current->next->n)
			{
				...
				_swap_nodes(list, current->prev, current);
				print_list(*list);
			}
			else
			{
		...
		while (current->prev != start && current->prev != NULL)
		{
			...
			{
				current = current->prev;
				swapped = _swap_nodes(list, current, current->next);
				print_list(*list);
			}
			else
			{
...
/**
 * _swap_nodes - Swaps the nodes of d-linkedlist
 * @list: head pointer
 * @n1: node1 pointer
 * @n2: node2 pointer
 * Return: 1
 */
int _swap_nodes(listint_t **list, listint_t *n1, listint_t *n2)
{
	n1->next = n2->next;
